...
Each word is an atom, like Chinese or Vietnamese. It uses the sounds found in English and Spanish, with no tones. Longer words can be constructed to be multiple syllables, but they serve as atoms as well. There is no tense or plural changes to a word's morphology, they are separate atoms. Noun phrases, verb phrases, adjective phrases, and focuser phrases are formed by adding a specific word after the sequence of words.
...
- The word `mam` is for "all".
- The word `nan` is for "none".
- The word `nanmam` is for "few".
- The word `sUm` is for "some".
- The word `mamnan` is for "most".
- The word `mUs` is for "many".
- The word `nam` is "any".
...
> <img src="https://github.com/mountbuild/xoox/blob/build/examples/sentences/ma-U-vis-i-da-U-zok-o.png?raw=true"/>
